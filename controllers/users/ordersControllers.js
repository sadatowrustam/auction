...
const { Products, Orders, Orderproducts, Productsizes, Productcolor, Stock, Seller } = require('../../models');
...
exports.getNotOrderedProducts = catchAsync(async(req, res, next) => {
    const limit = req.query.limit || 20
    const offset = req.query.offset || 0
    const order_products = await Orderproducts.findAll({ where: { userId: req.user.id, is_ordered: false }, limit, offset })
    const checked_products = []
    for (var i = 0; i < order_products.length; i++) {
        const product = await Products.findOne({
            where: { product_id: order_products[i].product_id },
